...
async def grant_myth(ctx, member: discord.Member, myth: str):
    ...
    try:
        for other_myth in MYTHS.values():
            other_role = discord.utils.get(ctx.guild.roles, name=other_myth["role"])
            if other_role and other_role in member.roles and other_role != role_obj:
                await member.remove_roles(other_role, reason=f"!grant_myth by {ctx.author}")
        await member.add_roles(role_obj, reason=f"!grant_myth by {ctx.author}")
    except discord.Forbidden:
        await ctx.send(
...
async def tutorial(ctx):
    """DM a step-by-step guide for starting and using the bot."""
    try:
        await ctx.message.delete()
    except Exception:
        pass
