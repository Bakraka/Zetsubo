...
FEMBOY_ROLE       = "i SWEAR im not DL bro and i put that on my kids!"
...
def _empty_data() -> dict:
    return {
        ...
        "justice_trials":    {},     # trial_id -> justice trial state dict
        "appeals":           {},     # user_id -> latest mute/banishment appeal
    }
def get_user(data: dict, user_id: int) -> dict:
    ...
    if uid not in data["users"]:
        data["users"][uid] = {
            ...
            "standalone_role":    None,   # justice/prudence/etc.; never treated as a sin
            "fallen":             False,
            "muted_until":        None,
            "fallen_until":       None,
            "redemption_count":   0,
            ...
            "envy_ability_cds":       {},     # per-ability cooldowns (schizo, etc.)
            "wallet":                 100,    # shop currency — starting balance
            "invulnerable_until":     None,   # Justice trial protection
            "path_choice_deadline":   None,   # 3-min forced path-choice window
            ...
            # Optional fictional transformation mini-game. Participation is
            # opt-in and every transformation has an explicit opt-out.
            "femboy_opt_in":          False,
            "femboy_meter":            0,
            "femboy_meter_until":      None,
            "femboy_until":            None,
            "femboy_forced_until":     None,
            "femboy_master_id":         None,
            "femboy_master_commands":   [],
            "femboy_ability_cds":       {},
            "femboy_suggestion_uses":   [],
            "femboy_suggestion_cd":     None,
            "femboy_partner_id":        None,
            "femboy_bf_requests":       {},
            "femboy_rejection_uses":    5,
            "femboy_rejection_reset":   None,
            "femboy_confession_votes":  [],
            "femboy_confession_open":   False,
            "femboy_second_form":       False,
            "radioactive_charge":       0,
        }
    else:
        u = data["users"][uid]
        u.setdefault("standalone_role",    None)
        u.setdefault("muted_until",        None)
        u.setdefault("marks_of_insecurity",    0)
        u.setdefault("envy_ability_cds",       {})
        u.setdefault("wallet",                 100)
        u.setdefault("invulnerable_until",     None)
        u.setdefault("path_choice_deadline",   None)
        ...
        u.setdefault("femboy_opt_in",          False)
        u.setdefault("femboy_meter",            0)
        u.setdefault("femboy_meter_until",      None)
        u.setdefault("femboy_until",            None)
        u.setdefault("femboy_forced_until",     None)
        u.setdefault("femboy_master_id",         None)
        u.setdefault("femboy_master_commands",   [])
        u.setdefault("femboy_ability_cds",       {})
        u.setdefault("femboy_suggestion_uses",   [])
        u.setdefault("femboy_suggestion_cd",     None)
        u.setdefault("femboy_partner_id",        None)
        u.setdefault("femboy_bf_requests",       {})
        u.setdefault("femboy_confession_open",   False)
        u.setdefault("femboy_rejection_uses",    5)
        u.setdefault("femboy_rejection_reset",   None)
        u.setdefault("femboy_confession_votes",  [])
        u.setdefault("femboy_second_form",       False)
        u.setdefault("radioactive_charge",       0)
...
TUTORIAL_CHANNEL_NAME = "tutorial"

async def tutorial_channel(guild: discord.Guild) -> Optional[discord.TextChannel]:
    return discord.utils.get(guild.text_channels, name=TUTORIAL_CHANNEL_NAME)
...
STOLEN_ABILITY_DURATION     = 30 * 60   # Fallback if target holds no sin role
STEAL_ABILITY_COOLDOWN      = 30 * 60   # Cooldown between !steal_ability attempts
...
PATH_CHOICE_WINDOW    = 3 * 60      # 3 minutes to !choose_path before a default is assigned
DEFAULT_PATH          = "hybrid"    # assigned automatically if the window expires
...
def power_scaled_secs(power: int, low_secs: int, high_secs: int) -> int:
    """
    Scale a duration between low_secs and high_secs across the 1-7 sin power range.
    Higher power → longer duration (closer to high_secs).
    """
    power = max(1, min(7, power))
    span  = high_secs - low_secs
    return int(low_secs + span * (power - 1) / 6)

def greed_steal_duration_secs(power: int) -> int:
    """How long Greed holds a stolen ability — 5-10 min based on the target's sin power."""
    return power_scaled_secs(power, 5 * 60, 10 * 60)

def devour_duration_secs(power: int) -> int:
    """How long a devoured target is silenced — 5-10 min based on who was devoured."""
    return power_scaled_secs(power, 5 * 60, 10 * 60)
...
async def grant_free_message_grace(member: discord.Member, data: dict, grace_seconds: int = 15):
    """
    Give a member one free, unrestricted message before an incoming mute actually
    lands (e.g. to call `!call_trial` on their attacker). The caller should apply
    the real mute effect only after this grace window via a delayed task.
    """
    user = get_user(data, member.id)
    user["mute_grace_until"] = now_ts() + grace_seconds
    user["mute_grace_used"]  = False
    save_data(data)
    try:
        await member.send(
            f"⚠️ You're about to be muted. You have **{grace_seconds} seconds** and "
            "**one free message** first — for example `!call_trial @user` to demand a trial."
        )
    except Exception:
        pass

async def handle_pvp_death(member: discord.Member, data: dict, ch, slain_by: str):
    """
    A PVP/PVE death (not a trial/virtue failure). Per design: dying no longer
    strips roles or applies Fallen — the member keeps every role, but is sent
    to the Land of Impurity (banishment) until they !pry their way out.
    """
    user = get_user(data, member.id)
    user["pvp_deaths"] = user.get("pvp_deaths", 0) + 1
    user["hp"] = 0
    save_data(data)
    if ch:
        await ch.send(embed=discord.Embed(
            title="☠️ Struck Down",
            description=f"{member.mention} has fallen in combat ({slain_by}).",
            color=discord.Color.dark_gray(),
        ))
    await send_to_banishment(member, data, slain_by)
...
async def complete_trial(member: discord.Member, sin: str, data: dict = None):
    ...
    user["wallet"] = user.get("wallet", 100) + 75  # trial-completion shop currency
    _reset_trial_progress(user)
    ...
    if ch:
        ...
        view = build_ability_view(member.id, sin)
        if view:
...
async def complete_virtue_trial(member: discord.Member, sin: str, data: dict = None):
    ...
    user["wallet"] = user.get("wallet", 100) + 75  # virtue-trial-completion shop currency
...
async def kill(ctx, target: discord.Member):
    ...
    target_user["muted_until"] = until.timestamp()
...
async def attack(ctx, target: discord.Member = None):
    ...
    # Justice trial invulnerability window
    if now_ts() < (d_user.get("invulnerable_until") or 0):
        await ctx.send(
            f"{target.display_name} is **invulnerable** from a recent Justice ruling.",
            delete_after=6,
        )
        save_data(data)
        return
    ...
    if d_user.get("hp", d_stats[0]) <= 0:
        ...
        a_user["wallet"]    = a_user.get("wallet", 100) + 25  # combat-kill shop currency
        ...
        save_data(data)
        await handle_pvp_death(target, data, ch, f"defeated by {ctx.author.display_name}")
...
async def sister_kill(ctx, target: discord.Member = None):
    ...
    if d_user.get("hp", d_stats[0]) <= 0:
        await handle_pvp_death(target, data, ch, "slain by the Despair Sister")
...
async def student_attack(ctx, target: discord.Member = None):
    ...
    if d_user.get("hp", d_stats[0]) <= 0:
        await handle_pvp_death(target, data, ch, "defeated by a Reserve Course Student")
...
async def setup(ctx):
    ...
    await make_role(FEMBOY_ROLE, discord.Color.from_rgb(255, 125, 190), hoist=False)
    ...
    # ── Banished role: denies Send Messages / Add Reactions server-wide by
    # default, so it silences someone everywhere except an explicit channel
    # override on #banished-place below. ──
    if BANISHED_ROLE_NAME not in existing_role_names:
        try:
            await guild.create_role(
                ...
                reason="!setup — Seven Sins bot",
            )
            created_roles.append(BANISHED_ROLE_NAME)
            existing_role_names.add(BANISHED_ROLE_NAME)
        except discord.Forbidden:
            created_roles.append(f"~~{BANISHED_ROLE_NAME}~~ *(missing permission)*")
        except Exception as e:
            errors.append(f"Role {BANISHED_ROLE_NAME}: {type(e).__name__}")
    else:
        existing_roles.append(BANISHED_ROLE_NAME)
    ...
    await ensure_channel(TUTORIAL_CHANNEL_NAME, "DESPAIR")

    # Banished-place: visible to everyone, but only the Banished role (and admins)
    # can actually send in it — that's how their server-wide mute gets its one exception.
    banished_role_obj = discord.utils.get(guild.roles, name=BANISHED_ROLE_NAME)
    banished_overwrites = {
        guild.default_role: discord.PermissionOverwrite(read_messages=True, send_messages=False),
        guild.me: discord.PermissionOverwrite(read_messages=True, send_messages=True, manage_messages=True),
    }
    if banished_role_obj:
        banished_overwrites[banished_role_obj] = discord.PermissionOverwrite(
            read_messages=True, send_messages=True,
        )
    await ensure_channel(BANISHED_CHANNEL_NAME, "DESPAIR", banished_overwrites)

    if BANISHED_VC_NAME not in {c.name for c in guild.voice_channels}:
        try:
            vc_overwrites = {
                guild.default_role: discord.PermissionOverwrite(view_channel=True, connect=True),
            }
            cat = category_map.get("DESPAIR")
            await guild.create_voice_channel(
                BANISHED_VC_NAME, category=cat, overwrites=vc_overwrites,
                reason="!setup — Seven Sins bot",
            )
            created_channels.append(BANISHED_VC_NAME)
        except discord.Forbidden:
            created_channels.append(f"~~{BANISHED_VC_NAME}~~ *(missing permission)*")
        except Exception as e:
            errors.append(f"Voice channel {BANISHED_VC_NAME}: {type(e).__name__}")
    else:
        existing_channels.append(BANISHED_VC_NAME)
...
async def grant_special(ctx, member: discord.Member, role_key: str):
    ...
    data = load_data()
    user = get_user(data, member.id)
    # Standalone virtues are additive roles. Do not write their key into
    # sin_role: doing so makes virtue holders look like a sin holder and
    # breaks sin-only abilities and the virtue display.
    user["standalone_role"] = key
    user["fallen"] = False
    try:
...
@bot.command(name="grant_any_role")
@commands.has_permissions(administrator=True)
async def grant_any_role(ctx, member: discord.Member, *, role_name: str):
    """Admin: assign ANY existing server role to a member (bypasses the sin/virtue system entirely).
    If the role doesn't exist yet, it is created first."""
    bot_member = ctx.guild.me
    if bot_member is None:
        await ctx.send("❌ I cannot verify my role position right now.", delete_after=8)
        return
    ...
    if not role_obj:
        try:
            role_obj = await ctx.guild.create_role(
                name=role_name, reason=f"!grant_any_role by {ctx.author}",
            )
            created = True
        except discord.Forbidden:
            await ctx.send(f"❌ Permission denied — cannot create **{role_name}**.", delete_after=8)
            return
        except Exception as e:
            await ctx.send(f"❌ Could not create **{role_name}**: {type(e).__name__}", delete_after=8)
            return

    if role_obj >= bot_member.top_role:
        await ctx.send(
            f"❌ **{role_name}** sits above my top role — drag my role higher in "
            "**Server Settings → Roles** and try again.",
            delete_after=10,
        )
        return

    try:
        await member.add_roles(role_obj, reason=f"!grant_any_role by {ctx.author}")
        await ctx.send(
            f"✅ {'Created and granted' if created else 'Granted'} **{role_name}** to {member.mention}."
        )
    except discord.Forbidden:
        await ctx.send(f"❌ Permission denied while assigning **{role_name}**.", delete_after=8)
    except discord.HTTPException as e:
        await ctx.send(f"❌ Discord API error: `{e.status} — {e.text}`", delete_after=8)


# Every "meter"/charge-bar field in the game, and what counts as its max/ready value.
# Used by !fill_meter to instantly charge someone's ultimate, whatever role they hold.
METER_MAX_VALUES: dict[str, int] = {
    "anger_meter":        100,   # Greed → Frenzy / "I always get what I WANT!"
    "laziness_meter":     100,   # Sloth → forced Deep Sleep threshold
    "obsession_meter":    100,   # Lust → obsession ultimate
    "marks_of_insecurity": 6,    # Envy/Pride → Krodingers Effect lockout trigger (uses lowest threshold)
    "pride_recognition":  PRIDE_RECOGNITION_THRESHOLD,  # Pride → Stop Time unlock
    "gooner_images_submitted": 100,  # Gooner trial ultimate unlock
    "corruption":         5,     # generic corruption-gated thresholds some abilities check
}


@bot.command(name="fill_meter")
@commands.has_permissions(administrator=True)
async def fill_meter(ctx, member: discord.Member, meter: str = "auto"):
    """Admin: instantly max out a member's ultimate/charge meter, whichever one matches their role.
    Usage: !fill_meter @user [meter_name] — omit meter_name to auto-detect from their sin role."""
    data = load_data()
    user = get_user(data, member.id)

    auto_map = {
        "greed":  "anger_meter",
        "sloth":  "laziness_meter",
        "lust":   "obsession_meter",
        "envy":   "marks_of_insecurity",
        "pride":  "pride_recognition",
        "gooner": "gooner_images_submitted",
    }

    if meter == "auto":
        sin = user.get("sin_role")
        meter = auto_map.get(sin)
        if not meter:
            await ctx.send(
                f"❌ {member.mention} doesn't hold a role with an auto-detectable meter. "
                f"Specify one manually: `{'`, `'.join(METER_MAX_VALUES.keys())}`.",
                delete_after=12,
            )
            save_data(data)
            return
        ...
        meter = meter.lower()
        if meter not in METER_MAX_VALUES:
            await ctx.send(
                f"❌ Unknown meter `{meter}`. Valid: `{'`, `'.join(METER_MAX_VALUES.keys())}`.",
                delete_after=10,
            )
            save_data(data)
            return

    max_val = METER_MAX_VALUES[meter]
    user[meter] = max_val

    # Some meters need a companion flag flipped to actually "unlock" the ultimate
    followups = []
    if meter == "anger_meter":
        user["frenzy_active"] = True
        user["frenzy_used"]   = False
        followups.append("Frenzy is now **active** — they can use `!i_always_get_what_i_want` / `!frenzy_clash`.")
    elif meter == "pride_recognition":
        user["stop_time_unlocked"] = True
        followups.append("Stop Time is now **unlocked**.")
    elif meter == "laziness_meter":
        followups.append("Their next qualifying action will trigger a forced Deep Sleep.")

    save_data(data)
    desc = f"{member.mention}'s **{meter.replace('_', ' ')}** has been set to **{max_val}**."
    if followups:
        desc += "\n\n" + "\n".join(followups)
    await ctx.send(embed=discord.Embed(title="⚡ Meter Filled", description=desc, color=discord.Color.gold()))
...
async def _send_tutorial_pages(ctx, pages: list[discord.Embed]):
    """Post tutorial pages to the server's #tutorial channel (created by !setup) instead of DMing."""
    ch = await tutorial_channel(ctx.guild) if ctx.guild else None
    if not ch:
        raise LookupError("no tutorial channel")
    for page in pages:
...
async def _run_category_tutorial(ctx, page_index: int):
    try:
        ...
        await ctx.send(
            f"❌ No **#{TUTORIAL_CHANNEL_NAME}** channel found. Run `!setup` first to create it, "
            "then try the tutorial command again.",
            delete_after=10,
        )
...
async def obtainment(ctx):
    """Post obtainment methods for every role category to #tutorial."""
    try:
        await _send_tutorial_pages(ctx, _category_tutorial_pages())
    except LookupError:
        await ctx.send(
            f"❌ No **#{TUTORIAL_CHANNEL_NAME}** channel found. Run `!setup` first, "
            "then try `!obtainment` again.",
            delete_after=10,
        )
...
async def tutorial(ctx):
    """Post the starter guide plus every category's powers and obtainment methods to #tutorial."""
    try:
        await ctx.message.delete()
    except Exception:
        pass
    ...
    play_embed.add_field(
        name="Sin abilities",
        value=(
            "Use `!commands` for the complete command list, or `!abilities` for just "
            "your own current role's powers. Most abilities require the matching sin "
            "role and have cooldowns."
        ),
        inline=False,
    )
    play_embed.add_field(
    ...
    play_embed.add_field(
        ...
        value=(
            "After these starter pages, this tutorial also posts the complete "
            "categorized command manual here. It covers every listed power for Lust, "
    ...
    play_embed.add_field(
        ...
        value=(
            "Admins can use `!grant`, `!grant_virtue`, `!grant_any_role`, `!fill_meter`, "
            "`!force_fall`, `!reset_user`, and `!strip_all @user`. `!strip_all` affects "
            "only the one member named."
        ),
        inline=False,
    )

    try:
        await ch.send(embed=setup_embed)
        await ch.send(embed=play_embed)
        await _send_tutorial_pages(ctx, _category_tutorial_pages())
        ...
        await ctx.send(
            f"❌ I don't have permission to post in **#{TUTORIAL_CHANNEL_NAME}**. "
            "Check my channel permissions and run `!tutorial` again.",
            delete_after=10,
        )
...
async def grant_virtue(ctx, member: discord.Member, sin: str):
    ...
    if sin not in VIRTUES:
        ...
        return
    data = load_data()
    # Virtues are tied to the matching sin role (a virtue is that sin's redemption arc),
    # so sin_role must match the virtue being granted or its abilities will silently
    # never pass _virtue_check. Always align it here instead of only "if missing".
    user = get_user(data, member.id)
    previous_sin = user.get("sin_role")
    overwrote_different_sin = bool(previous_sin) and previous_sin != sin
    user["sin_role"] = sin
    ...
    try:
        ...
        if overwrote_different_sin:
            msg += (
                f"\n⚠️ {member.mention} previously held the **{previous_sin.capitalize()}** "
                f"sin role internally — that has been switched to **{sin.capitalize()}** "
                "so this virtue's abilities work correctly. Their old sin role in Discord "
                "was not removed automatically; remove it manually if it shouldn't overlap."
            )
        await ctx.send(msg)
    except discord.Forbidden:
        await ctx.send(
...
# ───────────────────────────────────────────────────────────────────
# COMMAND: !schizo @user  (Envy — Schizophrenia ability, always lands — indirect effect, no clash)
# ───────────────────────────────────────────────────────────────────

@bot.command()
async def schizo(ctx, target: discord.Member):
    """Envy ability: flood the channel with fake messages as if from other server members. No clash — indirect effects always land."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    ...
    cd = user.setdefault("envy_ability_cds", {}).get("schizo", 0) or 0
    if now_ts() < cd:
        await ctx.send(f"Schizo on cooldown: {remaining_fmt(cd)}.", delete_after=8)
        save_data(data)
        return
    ...
    user["envy_ability_cds"]["schizo"] = now_ts() + 15 * 60   # 15-min CD

    ch = await trial_channel(ctx.guild) or ctx.channel
    await ch.send(embed=discord.Embed(
        title="👁️ Envy Strikes",
        description=f"**Envy** turns its gaze on {target.mention}. **Reality fractures…**",
        color=discord.Color.from_rgb(0, 180, 216),
    ))
...
async def devour(ctx, target: discord.Member):
    """(Gluttony) Swallow a player whole — they cannot send any messages or commands for 5-10 minutes, depending on who they devoured."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    if not _gluttony_role_check(user):
    ...
    target_sin    = target_user.get("sin_role")
    target_power  = SINS[target_sin]["power"] if target_sin and target_sin in SINS else 1
    devour_secs   = devour_duration_secs(target_power)
    devour_mins   = devour_secs // 60
    grace_secs    = 15
    devoured_until = now_ts() + grace_secs + devour_secs

    user["gluttony_ability_cds"]["devour"] = now_ts() + 3 * 3600   # 3hr CD
    save_data(data)
    await grant_free_message_grace(target, data, grace_secs)
    ...
    await ch.send(embed=discord.Embed(
        ...
        description=(
            ...
            f"🤐 {target.mention} has **{grace_secs} seconds** and one free message, then cannot send "
            f"any messages or commands for **{devour_mins} minutes**.\n"
            f"Full effect ends: {ts_fmt(devoured_until)}"
        ),
        color=discord.Color.from_rgb(160, 50, 0),
    ))

    async def _apply_devour_mute():
        await asyncio.sleep(grace_secs)
        d2 = load_data()
        tu2 = get_user(d2, target.id)
        tu2["devoured_until"] = now_ts() + devour_secs
        save_data(d2)
    asyncio.create_task(_apply_devour_mute())
    try:
        await target.send(
            f"🌑 **{ctx.author.display_name}** devoured you. "
            f"You cannot send any messages for **{devour_mins} minutes**. Sit tight."
        )
    except Exception:
        pass

@bot.command(name="purge_bite")
async def purge_bite(ctx, target: discord.Member, count: int = 1):
    """(Gluttony) Chew up a target's recent messages — deletes their last 1-10 messages in this channel. 20-min CD."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    if not _gluttony_role_check(user):
        await ctx.send("Only the Devoured may purge a meal.", delete_after=5); save_data(data); return

    cd = user["gluttony_ability_cds"].get("purge_bite", 0) or 0
    if now_ts() < cd:
        await ctx.send(f"Purge Bite on cooldown: {remaining_fmt(cd)}.", delete_after=8)
        save_data(data); return
    if target.id == ctx.author.id:
        await ctx.send("You cannot purge yourself.", delete_after=5); save_data(data); return

    count = max(1, min(10, count))
    user["gluttony_ability_cds"]["purge_bite"] = now_ts() + 20 * 60
    save_data(data)
    ...
    await ch.send(embed=discord.Embed(
        title="🍽️ Purge Bite",
        description=f"{ctx.author.mention} devoured **{deleted}** of {target.mention}'s recent message(s).",
        color=discord.Color.from_rgb(160, 50, 0),
    ))

@bot.command(name="expel")
async def expel(ctx, target: discord.Member, minutes: int = 5):
    """(Gluttony) Spit a target out of the server for 5-10 minutes — they're timed out and disconnected
    from voice, then automatically restored. This is a temporary removal, not a real Discord kick
    (a real kick is permanent and would need a fresh invite to undo). 2hr CD."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    if not _gluttony_role_check(user):
        await ctx.send("Only the Devoured may expel someone.", delete_after=5); save_data(data); return

    cd = user["gluttony_ability_cds"].get("expel", 0) or 0
    if now_ts() < cd:
        await ctx.send(f"Expel on cooldown: {remaining_fmt(cd)}.", delete_after=8)
        save_data(data); return
    if target.id == ctx.author.id:
        await ctx.send("You cannot expel yourself.", delete_after=5); save_data(data); return
    if target.guild_permissions.administrator:
        await ctx.send("Invalid target.", delete_after=5); save_data(data); return

    minutes = max(5, min(10, minutes))
    user["gluttony_ability_cds"]["expel"] = now_ts() + 2 * 3600
    save_data(data)

    grace_secs = 15
    await grant_free_message_grace(target, data, grace_secs)
    await ctx.send(
        f"🚪 {target.mention} has **{grace_secs} seconds** and one free message before being expelled.",
        delete_after=grace_secs + 2,
    )

    async def _apply_expel():
        await asyncio.sleep(grace_secs)
        until = datetime.now(timezone.utc) + timedelta(minutes=minutes)
        try:
            await target.timeout(until, reason=f"!expel by {ctx.author} (Gluttony)")
        except Exception:
            return
        try:
            if target.voice:
                await target.move_to(None, reason="Expelled by Gluttony")
        except Exception:
            pass
        ch2 = await trial_channel(ctx.guild) or ctx.channel
        await ch2.send(embed=discord.Embed(
            title="🚪 EXPELLED",
            description=(
                f"{target.mention} has been thrown out of the server for **{minutes} minutes**.\n\n"
                "They're timed out and disconnected from voice — full access returns automatically."
            ),
            color=discord.Color.from_rgb(160, 50, 0),
        ))
    asyncio.create_task(_apply_expel())
...
async def steal_ability(ctx, target: discord.Member):
    ...
    The cooldown is exactly the same duration as the stolen ability lock:
    5-10 minutes based on the target's power.
    """
    data = load_data()
    user = get_user(data, ctx.author.id)
    ...
    cd = user.setdefault("greed_ability_cds", {}).get("steal_ability", 0) or 0
    if now_ts() < cd:
        await ctx.send(f"Steal Ability on cooldown: {remaining_fmt(cd)}.", delete_after=8)
        save_data(data); return
    ...
    target_power_lvl = SINS[target_sin]["power"]
    steal_duration    = greed_steal_duration_secs(target_power_lvl)  # also used as the cooldown
    ...
    user["greed_ability_cds"]["steal_ability"] = now_ts() + steal_duration
...
_SCALE_ABILITY_CD_MAP = {
    ...
    "schizo":           "envy_ability_cds",
    "envy_strike":      "envy_ability_cds",
...
def _standalone_virtue_check(user: dict, key: str) -> bool:
    ...
    return user.get("standalone_role") == key and not user.get("fallen")
...
async def inspire_strike(ctx, target: discord.Member):
    ...
    if winner == "a":
        ...
        all_cd_dicts = ["virtue_ability_cds", "justice_ability_cds", "gluttony_ability_cds",
                        ...
                        "envy_ability_cds", "gooner_ability_cds", "hope_ability_cds", "faith_ability_cds",
...
async def summon_meteor(ctx, target: discord.Member):
    ...
    # Reverence aura reduces meteor's severity slightly (still always lands — AoE/ultimate: no clash)
    ...
    penalty_amt = max(1, 2 - aura_penalty)
    lock_secs   = max(0, 600 - aura_penalty * 120)
    target_user["clash_power_penalty"] = (target_user.get("clash_power_penalty") or 0) + penalty_amt
    target_user["clash_penalty_until"] = now_ts() + 3600
    if lock_secs:
        target_user["ability_locked_until"] = now_ts() + lock_secs
    target_user["corruption"] = target_user.get("corruption", 0) + 1
    save_data(data)

    flip_embed = discord.Embed(
        title="☄️ Meteor Strike",
        description=(
            f"☄️ **DIRECT HIT.** An ultimate/AoE strike cannot be clashed away.\n\n"
            f"{target.mention} is **obliterated**:\n"
            f"• **-{penalty_amt} clash power** for **1 hour**\n"
            + (f"• **Ability locked** for **{lock_secs // 60} minutes**\n" if lock_secs else "")
    ...
    try:
        await target.send(
            f"☄️ A **METEOR** struck you directly. "
            f"-{penalty_amt} clash power for 1 hour"
            + (f", abilities locked for {lock_secs // 60} min" if lock_secs else "")
            + ", +1 Corruption."
        )
    except Exception:
        pass
...
# ───────────────────────────────────────────────────────────────────
# ═══════════════════════════════════════════════════════════════════
# ░░ MUTE / BANISHMENT APPEALS ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
# ═══════════════════════════════════════════════════════════════════

@bot.command(name="appeal")
async def appeal(ctx, *, reason: str = "No reason provided."):
    """Submit an appeal while muted, timed out, or banished."""
    data = load_data()
    data.setdefault("appeals", {})[str(ctx.author.id)] = {
    ...
    save_data(data)
    await ctx.send(
        "⚖️ Your appeal was submitted for admin review. "
        "Trial status and allowed appeal actions remain available.",
        delete_after=10,
    )


@bot.command(name="review_appeals")
@commands.has_permissions(administrator=True)
async def review_appeals(ctx):
    """Admin: view pending mute/banishment appeals."""
    data = load_data()
    pending = [
        appeal_data for appeal_data in data.get("appeals", {}).values()
        if appeal_data.get("status") == "pending"
    ]
    if not pending:
        await ctx.send("No pending appeals.", delete_after=6)
        return
    lines = [
        f"• <@{item['user_id']}> — `{item['reason'][:160]}`"
        for item in pending[:20]
    ]
    await ctx.send(
        embed=discord.Embed(
            title="⚖️ Pending Appeals",
            description="\n".join(lines)
            + "\n\nResolve one with `!resolve_appeal @user approve|deny`.",
            color=discord.Color.gold(),
        )
    )


@bot.command(name="resolve_appeal")
@commands.has_permissions(administrator=True)
async def resolve_appeal(ctx, member: discord.Member, decision: str):
    """Admin: approve or deny a submitted appeal."""
    decision = decision.lower().strip()
    if decision not in {"approve", "deny"}:
        await ctx.send("Choose `approve` or `deny`.", delete_after=5)
        return
    data = load_data()
    item = data.setdefault("appeals", {}).get(str(member.id))
    if not item or item.get("status") != "pending":
        await ctx.send("That member has no pending appeal.", delete_after=6)
        return
    item["status"] = "approved" if decision == "approve" else "denied"
    item["resolved_at"] = now_ts()
    item["resolved_by"] = str(ctx.author.id)
    if decision == "approve":
        target_user = get_user(data, member.id)
        target_user["muted_until"] = None
        try:
            await member.timeout(None, reason=f"Appeal approved by {ctx.author}")
        except (discord.Forbidden, discord.HTTPException):
            pass
    save_data(data)
    await ctx.send(
        f"✅ Appeal for {member.mention} was **{decision}d**.",
        delete_after=8,
    )


# ═══════════════════════════════════════════════════════════════════
# ░░ OPTIONAL FICTIONAL FEMBOY MINI-GAME ░░░░░░░░░░░░░░░░░░░░░░░░░░░
# ═══════════════════════════════════════════════════════════════════
#
# This system is deliberately opt-in. A member must opt in before they can
# be targeted, every active state can be canceled, and no Discord role is
# assigned until the fictional 100-second meter completes.

FEMBOY_VIDEO_URL = "https://www.youtube.com/watch?v=DS9wcvy2YQM"
FEMBOY_DURATION = 5 * 60 * 60
FEMBOY_METER_SECONDS = 100
FEMBOY_RADIOACTIVE_COOLDOWN = 3 * 60
FEMBOY_CANCEL_PHRASES = (
...
FEMBOY_REQUIRED_WORDS = (
    "ahh i'm such a good boy!",
    "ahh im such a good boy!",
    "can i get pets now :3",
    ":3",
    "owo",
    "ow o",
    ">w<",
...
def _femboy_is_opted_in(user: dict) -> bool:
    return bool(user.get("femboy_opt_in")) and not user.get("femboy_opt_out_at")


def _reset_femboy_state(user: dict) -> None:
    user["femboy_meter"] = 0
    user["femboy_meter_until"] = None
    user["femboy_until"] = None
    user["femboy_forced_until"] = None
    user["femboy_master_id"] = None
    user["femboy_master_commands"] = []
    user["femboy_second_form"] = False


def _femboy_rejections_available(user: dict) -> int:
    reset = user.get("femboy_rejection_reset") or 0
    if now_ts() >= reset:
        user["femboy_rejection_uses"] = 5
        user["femboy_rejection_reset"] = now_ts() + 86400
    return max(0, int(user.get("femboy_rejection_uses", 5)))


async def _remove_femboy_role(member: discord.Member) -> None:
    role = discord.utils.get(member.guild.roles, name=FEMBOY_ROLE)
    if role and role in member.roles:
        try:
            await member.remove_roles(role, reason="Fictional femboy opt-out")
        except (discord.Forbidden, discord.HTTPException):
            pass


@bot.command(name="femboy_opt_in")
async def femboy_opt_in(ctx):
    """Opt into the fictional, consent-based femboy mini-game."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    user["femboy_opt_in"] = True
    user["femboy_opt_out_at"] = None
    _femboy_rejections_available(user)
    save_data(data)
    await ctx.send(
        "✅ You opted into the fictional femboy mini-game. "
        "You can cancel at any time with `!femboy_opt_out` or an opt-out phrase.",
        delete_after=12,
    )


@bot.command(name="femboy_opt_out")
async def femboy_opt_out(ctx):
    """Leave the fictional femboy mini-game and clear its active state."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    user["femboy_opt_in"] = False
    user["femboy_opt_out_at"] = now_ts()
    _reset_femboy_state(user)
    save_data(data)
    if ctx.guild:
        await _remove_femboy_role(ctx.author)
    await ctx.send("✅ You opted out. All fictional femboy states were cleared.", delete_after=8)


@bot.command(name="femboy_status")
async def femboy_status(ctx):
    """Show only the caller's private opt-in and meter state."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    meter = user.get("femboy_meter", 0)
    meter_until = user.get("femboy_meter_until") or 0
    state_until = user.get("femboy_until") or 0
    await ctx.author.send(
        embed=discord.Embed(
            title="🎀 Fictional Femboy Status",
            color=discord.Color.from_rgb(255, 125, 190),
            description=(
                f"Opted in: **{'yes' if _femboy_is_opted_in(user) else 'no'}**\n"
                f"Meter: **{meter}/{FEMBOY_METER_SECONDS}**"
                + (f" (running for {remaining_fmt(meter_until)})" if meter_until and now_ts() < meter_until else "")
                + "\n"
                + (
                    f"Active for {remaining_fmt(state_until)}."
                    if state_until and now_ts() < state_until
                    else "No active transformation."
                )
            ),
        )
    )
...
@bot.command(name="secretly_watch_femboys", aliases=["watch_femboys"])
async def secretly_watch_femboys(ctx):
    """Opt-in flavor ability: send the video privately and grant one charge."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    if not _femboy_is_opted_in(user):
        await ctx.send("Opt in first with `!femboy_opt_in`.", delete_after=6)
        save_data(data)
        return
    cd = user.setdefault("femboy_ability_cds", {}).get("watch_femboys", 0) or 0
    if now_ts() < cd:
        await ctx.send(f"Watch Femboys is on cooldown: {remaining_fmt(cd)}.", delete_after=8)
        save_data(data)
        return
    user["femboy_ability_cds"]["watch_femboys"] = now_ts() + 15 * 60
    user["radioactive_charge"] = min(3, user.get("radioactive_charge", 0) + 1)
    save_data(data)
    try:
        await ctx.author.send(
            f"🎀 Your private fictional link: {FEMBOY_VIDEO_URL}\n"
            "You received **1 radioactive substance charge**. "
            "Use `!radioactive_substance @user stun|damage` on another opted-in player."
        )
        await ctx.message.delete()
    except Exception:
        await ctx.send(
            "I could not DM the link. Enable DMs from server members and try again.",
            delete_after=10,
        )


@bot.command(name="radioactive_substance")
async def radioactive_substance(ctx, target: discord.Member, effect: str = "stun"):
    """Use one opt-in fictional charge for a stun or fixed 40 damage."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    target_user = get_user(data, target.id)
    if not _femboy_is_opted_in(user) or not _femboy_is_opted_in(target_user):
        await ctx.send("Both players must opt into the fictional mini-game first.", delete_after=8)
        save_data(data)
        return
    if target.id == ctx.author.id:
        await ctx.send("Choose another opted-in player.", delete_after=5)
        save_data(data)
        return
    if user.get("radioactive_charge", 0) < 1:
        await ctx.send("You have no radioactive substance charges. Use `!watch_femboys` first.", delete_after=8)
        save_data(data)
        return
    cd = user.setdefault("femboy_ability_cds", {}).get("radioactive_substance", 0) or 0
    if now_ts() < cd:
        await ctx.send(f"Radioactive Substance is on cooldown: {remaining_fmt(cd)}.", delete_after=8)
        save_data(data)
        return
    effect = effect.lower().strip()
    if effect not in {"stun", "damage"}:
        await ctx.send("Choose `stun` or `damage`.", delete_after=5)
        save_data(data)
        return

    user["radioactive_charge"] -= 1
    user["femboy_ability_cds"]["radioactive_substance"] = now_ts() + FEMBOY_RADIOACTIVE_COOLDOWN
    if effect == "stun":
        target_user["stunned_until"] = max(target_user.get("stunned_until", 0) or 0, now_ts() + 30)
        result = f"{target.mention} is **stunned for 30 seconds**."
    else:
        stats = get_role_stats(target)
        target_user["hp"] = max(0, target_user.get("hp", stats[0]) - 40)
        result = f"{target.mention} takes **40 fictional damage** ({target_user['hp']}/{stats[0]} HP)."
    save_data(data)
    await ctx.send(
        embed=discord.Embed(
            title="☢️ Radioactive Substance",
            description=f"{ctx.author.mention} used an opt-in fictional charge.\n{result}",
            color=discord.Color.green(),
        )
    )


async def _complete_femboy_meter(guild: discord.Guild, target: discord.Member, master_id: int):
    data = load_data()
    target_user = get_user(data, target.id)
    if not _femboy_is_opted_in(target_user):
        return
    target_user["femboy_meter"] = FEMBOY_METER_SECONDS
    target_user["femboy_meter_until"] = None
    target_user["femboy_until"] = now_ts() + FEMBOY_DURATION
    target_user["femboy_forced_until"] = target_user["femboy_until"]
    target_user["femboy_master_id"] = str(master_id)
    target_user["femboy_second_form"] = True
    role = discord.utils.get(guild.roles, name=FEMBOY_ROLE)
    if role:
        try:
            await target.add_roles(role, reason="Opt-in fictional femboy transformation")
        except (discord.Forbidden, discord.HTTPException):
            pass
    save_data(data)
    ch = await trial_channel(guild)
    if ch:
        await ch.send(
            embed=discord.Embed(
                title="🎀 Second Form Unlocked",
                description=(
                    f"{target.mention} completed the **opt-in fictional 100-second meter**.\n"
                    f"The state lasts **5 hours** and qualifying messages include one of: "
                    f"{', '.join(FEMBOY_REQUIRED_WORDS)}.\n"
                    "They can cancel at any time with `!femboy_opt_out`."
                ),
                color=discord.Color.from_rgb(255, 125, 190),
            )
        )
...
async def _run_femboy_meter(guild: discord.Guild, target: discord.Member, master_id: int):
    for second in range(FEMBOY_METER_SECONDS):
        await asyncio.sleep(1)
        data = load_data()
        target_user = get_user(data, target.id)
        if not _femboy_is_opted_in(target_user) or not target_user.get("femboy_meter_until"):
            return
        target_user["femboy_meter"] = second + 1
        save_data(data)
    await _complete_femboy_meter(guild, target, master_id)


@bot.command(name="femboy_suggestion", aliases=["i_would_like_you_better"])
async def femboy_suggestion(ctx, target: discord.Member):
    """Start the 100-second fictional meter for another opted-in member."""
    if ctx.guild is None:
        await ctx.send("This fictional mechanic requires a server.", delete_after=6)
        return
    data = load_data()
    user = get_user(data, ctx.author.id)
    target_user = get_user(data, target.id)
    if not _femboy_is_opted_in(user) or not _femboy_is_opted_in(target_user):
        await ctx.send("Both players must opt into this fictional mechanic first.", delete_after=8)
        save_data(data)
        return
    if target.id == ctx.author.id:
        await ctx.send("Choose another opted-in player.", delete_after=5)
        save_data(data)
        return
    suggestion_cd = user.get("femboy_suggestion_cd") or 0
    if now_ts() < suggestion_cd:
        await ctx.send(
            f"That fictional ability is on cooldown: {remaining_fmt(suggestion_cd)}.",
            delete_after=8,
        )
        save_data(data)
        return
    recent_uses = [
        ts for ts in user.get("femboy_suggestion_uses", [])
        if now_ts() - ts < 2 * 60 * 60
    ]
    if len(recent_uses) >= 2:
        await ctx.send("You can use this fictional ability twice every 2 hours.", delete_after=8)
        user["femboy_suggestion_uses"] = recent_uses
        save_data(data)
        return
    if target_user.get("femboy_meter_until") and now_ts() < target_user["femboy_meter_until"]:
        await ctx.send(f"{target.mention} already has a meter running.", delete_after=6)
        save_data(data)
        return
    target_user["femboy_meter"] = 0
    target_user["femboy_meter_until"] = now_ts() + FEMBOY_METER_SECONDS
    target_user["femboy_master_id"] = str(ctx.author.id)
    recent_uses.append(now_ts())
    user["femboy_suggestion_uses"] = recent_uses
    save_data(data)
    await ctx.send(
        f"🎀 {target.mention} opted in and their fictional meter started for **100 seconds**. "
        "They can cancel at any time with `!femboy_opt_out` or an opt-out phrase."
    )
    asyncio.create_task(_run_femboy_meter(ctx.guild, target, ctx.author.id))


@bot.command(name="femboy_reject")
async def femboy_reject(ctx):
    """Spend one of five daily fictional proposal rejections and cancel a meter."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    available = _femboy_rejections_available(user)
    if available <= 0:
        await ctx.send(
            "You have used all five daily fictional rejections. "
            "Use `!femboy_opt_out` to leave entirely.",
            delete_after=8,
        )
        save_data(data)
        return
    user["femboy_rejection_uses"] = available - 1
    master_id = user.get("femboy_master_id")
    if master_id:
        master = get_user(data, int(master_id))
        master["femboy_suggestion_cd"] = now_ts() + 2 * 60 * 60
    user["femboy_meter_until"] = None
    user["femboy_meter"] = 0
    user["femboy_master_id"] = None
    save_data(data)
    await ctx.send(
        f"❌ Fictional proposal rejected. **{available - 1}/5** daily rejections remain.",
        delete_after=8,
    )


@bot.command(name="pray_testosterone")
async def pray_testosterone(ctx):
    """Opt-in fictional counter-effect; only rolls during the configured daytime."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    if not _femboy_is_opted_in(user) or not (user.get("femboy_until") or 0) > now_ts():
        await ctx.send("You need an active fictional second form to pray.", delete_after=6)
        save_data(data)
        return
    cd = user.setdefault("femboy_ability_cds", {}).get("testosterone", 0) or 0
    if now_ts() < cd:
        await ctx.send(f"Testosterone prayer is on cooldown: {remaining_fmt(cd)}.", delete_after=8)
        save_data(data)
        return
    local_time = datetime.now(ZoneInfo("America/Chicago")).time()
    user["femboy_ability_cds"]["testosterone"] = now_ts() + 5 * 60
    if (7, 30) <= (local_time.hour, local_time.minute) <= (15, 30):
        if random.random() < 0.30:
            user["femboy_meter"] = max(0, user.get("femboy_meter", 100) - 20)
            result = "The prayer worked. The fictional meter dropped by **20%**."
        else:
            result = "The prayer was unanswered this time."
    else:
        result = "The prayer can only work between **7:30 AM and 3:30 PM Central**."
    save_data(data)
    await ctx.send(f"🙏 {result}", delete_after=10)


@bot.command(name="it_was_a_bet_bro")
async def it_was_a_bet_bro(ctx, target: discord.Member):
    """Opt-in fictional deflection for ordinary locks, stuns, and timeouts."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    target_user = get_user(data, target.id)
    if not _femboy_is_opted_in(user) or not _femboy_is_opted_in(target_user):
        await ctx.send("Both players must opt into the fictional mechanic.", delete_after=7)
        save_data(data)
        return
    cd = user.setdefault("femboy_ability_cds", {}).get("deflect", 0) or 0
    if now_ts() < cd:
        await ctx.send(f"Deflect is on cooldown: {remaining_fmt(cd)}.", delete_after=8)
        save_data(data)
        return
    user["femboy_ability_cds"]["deflect"] = now_ts() + 2 * 60 * 60
    target_user["muted_until"] = None
    target_user["stunned_until"] = None
    target_user["ability_locked_until"] = None
    target_user["envy_ability_locked_until"] = None
    try:
        await target.timeout(None, reason="Opt-in fictional deflection")
    except (discord.Forbidden, discord.HTTPException):
        pass
    save_data(data)
    await ctx.send(
        f"🎲 {ctx.author.mention} used **It Was A Bet, Bro** on {target.mention}. "
        "Ordinary fictional locks were cleared; ultimate/AoE effects are unaffected."
    )


@bot.command(name="femboy_request_bf")
async def femboy_request_bf(ctx, target: discord.Member):
    """Ask another opted-in player for a consensual fictional relationship."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    target_user = get_user(data, target.id)
    if target.id == ctx.author.id:
    ...
    if not _femboy_is_opted_in(user) or not _femboy_is_opted_in(target_user):
        ...
        save_data(data)
        return
    target_user.setdefault("femboy_bf_requests", {})[str(ctx.author.id)] = now_ts()
    save_data(data)
    await ctx.send(
        f"💌 {target.mention} received a fictional relationship request from {ctx.author.mention}. "
        f"They may use `!femboy_accept_bf {ctx.author.mention}` or `!femboy_deny_bf {ctx.author.mention}`."
    )


async def _clear_femboy_partner(guild: discord.Guild, user_id: int, partner_id: int):
    await asyncio.sleep(FEMBOY_DURATION)
    data = load_data()
    user = get_user(data, user_id)
    partner = get_user(data, partner_id)
    if user.get("femboy_partner_id") == str(partner_id):
        user["femboy_partner_id"] = None
    if partner.get("femboy_partner_id") == str(user_id):
        partner["femboy_partner_id"] = None
    save_data(data)
    member = guild.get_member(partner_id)
    if member:
        try:
            await member.edit(nick=None, reason="Fictional nickname expired")
        except (discord.Forbidden, discord.HTTPException):
            pass


@bot.command(name="femboy_accept_bf")
async def femboy_accept_bf(ctx, requester: discord.Member):
    """Accept a pending fictional relationship request."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    requester_user = get_user(data, requester.id)
    requests = user.setdefault("femboy_bf_requests", {})
    if str(requester.id) not in requests:
        await ctx.send("There is no pending request from that player.", delete_after=6)
        save_data(data)
        return
    if not _femboy_is_opted_in(user) or not _femboy_is_opted_in(requester_user):
        await ctx.send("Both players must remain opted in.", delete_after=7)
        save_data(data)
        return
    requests.pop(str(requester.id), None)
    user["femboy_partner_id"] = str(requester.id)
    requester_user["femboy_partner_id"] = str(ctx.author.id)
    nickname_note = ""
    if user.get("femboy_second_form"):
        try:
            await requester.edit(nick="la'bryant", reason="Consensual fictional nickname")
            nickname_note = " Their temporary nickname is **la'bryant**."
        except (discord.Forbidden, discord.HTTPException):
            nickname_note = " Discord did not allow the optional nickname change."
    save_data(data)
    asyncio.create_task(_clear_femboy_partner(ctx.guild, ctx.author.id, requester.id))
    await ctx.send(
        f"💞 {ctx.author.mention} accepted {requester.mention}'s fictional request.{nickname_note} "
        "Either person can opt out to end it."
    )


@bot.command(name="femboy_deny_bf")
async def femboy_deny_bf(ctx, requester: discord.Member):
    """Decline a pending fictional relationship request."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    user.setdefault("femboy_bf_requests", {}).pop(str(requester.id), None)
    save_data(data)
    await ctx.send(f"❌ {ctx.author.mention} declined the fictional request.", delete_after=8)


@bot.command(name="femboy_confess")
async def femboy_confess(ctx):
    """Open the five-person opt-in confession vote for a second form."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    if not _femboy_is_opted_in(user) or not user.get("femboy_second_form"):
        await ctx.send("You must be opted in and have completed the first fictional form.", delete_after=7)
        save_data(data)
        return
    user["femboy_confession_votes"] = [str(ctx.author.id)]
    user["femboy_confession_open"] = True
    save_data(data)
    await ctx.send(
        f"💗 {ctx.author.mention} opened a fictional confession vote. "
        f"Five opted-in members can use `!femboy_accept_confession {ctx.author.mention}`."
    )


@bot.command(name="femboy_accept_confession")
async def femboy_accept_confession(ctx, confessor: discord.Member):
    """Add an opted-in member's vote to a fictional confession."""
    data = load_data()
    voter = get_user(data, ctx.author.id)
    user = get_user(data, confessor.id)
    votes = user.get("femboy_confession_votes", [])
    if not user.get("femboy_confession_open"):
        await ctx.send("That fictional confession is not open.", delete_after=6)
        save_data(data)
        return
    if not _femboy_is_opted_in(voter) or str(ctx.author.id) in votes:
        await ctx.send("You must be opted in and may vote only once.", delete_after=7)
        save_data(data)
        return
    votes.append(str(ctx.author.id))
    if len(votes) >= 5:
        user["femboy_second_form"] = True
        user["femboy_confession_open"] = False
        result = f"{confessor.mention} reached five fictional confession votes and unlocked the second form."
    else:
        result = f"Confession vote recorded: **{len(votes)}/5**."
    save_data(data)
    await ctx.send(f"💗 {result}")


# Private target-command menu. It never executes a command for a user; it
# only gives that user a private mention-based template.
TARGET_MENU_COMMANDS = (
    ("attack", "!attack @user", "Universal combat attack"),
    ("radioactive", "!radioactive_substance @user stun|damage", "Opt-in fictional charge"),
    ("femboy suggestion", "!femboy_suggestion @user", "Opt-in fictional meter"),
    ("heal", "!heal", "Recover your HP"),
)


class TargetCommandSelect(discord.ui.Select):
    def __init__(self):
        super().__init__(
            placeholder="Choose a command template",
            min_values=1,
            max_values=1,
            options=[
                discord.SelectOption(label=label, value=syntax, description=description[:100])
                for label, syntax, description in TARGET_MENU_COMMANDS
            ],
        )

    async def callback(self, interaction: discord.Interaction):
        await interaction.response.send_message(
            f"Private template: `{self.values[0]}`\nReplace `@user` with the intended member mention.",
            ephemeral=True,
        )


class TargetCommandMenu(discord.ui.View):
    def __init__(self):
        super().__init__(timeout=120)
        self.add_item(TargetCommandSelect())


@bot.command(name="target_menu")
async def target_menu(ctx):
    """Show private clickable command templates that still require @targets."""
    await ctx.send(
        "Choose a command below. Only you see the follow-up template.",
        view=TargetCommandMenu(),
        delete_after=120,
    )
...
async def on_message(message: discord.Message):
    if message.author.bot:
        ...
        return
    # Commands may be used in DMs, but guild-only mechanics below need a
    # guild/channel. Avoid dereferencing message.guild for a DM.
    ...
    # Optional fictional mini-game cancellation is checked before every other
    # mechanic so an opt-out phrase always wins over a trial or command lock.
    content_low = content.lower()
    if (
        user.get("femboy_opt_in")
        and any(phrase in content_low for phrase in FEMBOY_CANCEL_PHRASES)
        and not content_low.startswith("!femboy_opt_out")
    ):
        user["femboy_opt_in"] = False
        user["femboy_opt_out_at"] = now_ts()
        _reset_femboy_state(user)
        save_data(data)
        await _remove_femboy_role(message.author)
        try:
            await message.delete()
        except Exception:
            pass
        try:
            await message.author.send("✅ Your fictional femboy state was canceled and you are opted out.")
        except Exception:
            pass
        return

    # A completed opt-in state requires one playful keyword in ordinary
    # messages. Commands remain available so the member can opt out.
    forced_until = user.get("femboy_forced_until") or 0
    if (
        forced_until > now_ts()
        and not is_cmd
        and not any(word in content_low for word in FEMBOY_REQUIRED_WORDS)
    ):
        try:
            await message.delete()
        except Exception:
        ...
        try:
            await message.author.send(
                "🎀 Your fictional second form is active. Include one of "
                f"{', '.join(FEMBOY_REQUIRED_WORDS)} or use `!femboy_opt_out`."
            )
        except Exception:
            pass
        return

    # Radioactive Substance's fictional stun prevents ordinary commands and
    # messages until it expires, while opt-out remains available.
    stunned_until = user.get("stunned_until") or 0
    if stunned_until > now_ts() and not content_low.startswith("!femboy_opt_out"):
        try:
            await message.delete()
        except Exception:
            pass
        try:
            await message.author.send(
                f"☢️ You are fictionally stunned for {remaining_fmt(stunned_until)}."
            )
        except Exception:
            pass
        return

    # Discord timeouts and the bot's own mute state block ordinary commands,
    # but trial status, closing arguments, banishment acceptance, appeals,
    # and opt-out remain available.
    muted_until = user.get("muted_until") or 0
    discord_timeout_until = getattr(message.author, "timed_out_until", None)
    discord_timeout_ts = discord_timeout_until.timestamp() if discord_timeout_until else 0
    if max(muted_until, discord_timeout_ts) > now_ts():
        allowed_while_muted = (
            "!call_trial",
            "!trial_status",
            "!closing_argument",
        ...
        if not content_low.startswith(allowed_while_muted):
            try:
                await message.delete()
            except Exception:
                pass
            try:
                await message.author.send(
                    f"🔇 You are muted until {remaining_fmt(max(muted_until, discord_timeout_ts))}. "
                    "Allowed: `!appeal`, `!trial_status`, and active trial/opt-out actions."
                )
            except Exception:
                pass
            return

    # ── MUTE GRACE: "one free message" before a pending mute actually lands ──
    # (see grant_free_message_grace / devour / expel / shop "silence")
    grace_until = user.get("mute_grace_until") or 0
    if now_ts() < grace_until and not user.get("mute_grace_used"):
        user["mute_grace_used"] = True
        save_data(data)
        # fall through — this one message (including a !call_trial) is allowed normally

    # ── JUSTICE TRIAL: fully-blackened participants are silenced except for
    # a small allowlist of trial commands ──
    if message.guild:
        _trial = data.get("justice_trials", {}).get(str(message.guild.id))
        if _trial and _trial.get("status") == "evidence":
            uid = str(message.author.id)
            if uid in (_trial["accuser_id"], _trial["defendant_id"]) and _trial["blackened"].get(uid, 0) >= 100:
                allowed = ("!closing_argument", "!trial_status", "!accept_banishment")
                if not content.lower().startswith(allowed):
                    try:
                        await message.delete()
                    except Exception:
                        pass
    ...
    # ── DEVOURED CHECK — Gluttony's devour blocks ordinary messages/commands.
    # Trial status, appeals, and opt-out remain available so a locked member
    # still has a recovery path.
    devoured_until = user.get("devoured_until") or 0
    if now_ts() < devoured_until:
        allowed_while_devoured = (
            "!call_trial",
            "!trial_status",
            "!closing_argument",
            "!accept_banishment",
        ...
        if content_low.startswith(allowed_while_devoured):
            if save_needed:
                save_data(data)
            await bot.process_commands(message)
            return
...
# ═══════════════════════════════════════════════════════════════════
# ░░ ABILITY CATEGORIES ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
# Metadata only — purely additive, does not change how any command works.
# ═══════════════════════════════════════════════════════════════════

ABILITY_CATEGORIES: dict[str, str] = {
    # Attack — direct single-target damage or debuff
    "attack": "Attack", "feast": "Attack", "rage_strike": "Attack",
    "force_lazy": "Attack", "slowdown": "Attack", "force_sleep": "Attack",
    "flash": "Attack", "withered_meat": "Attack", "jealousy_mark": "Attack",
    "jacobs_ladder": "Attack", "obsess": "Attack",
    # AoE / Ultimate — hits everyone, or is a sin's signature ultimate. No clash roll.
    "summon_meteor": "AoE/Ultimate", "disaster": "AoE/Ultimate",
    "izuru_despair": "AoE/Ultimate", "izuru_hope": "AoE/Ultimate",
    "stop_time": "AoE/Ultimate", "tragic_event": "AoE/Ultimate",
    "brainwash_remnant": "AoE/Ultimate",
    # DPS — repeatable damage-over-time / sustained pressure
    "devour": "DPS", "purge_bite": "DPS", "expel": "DPS",
    "sister_kill": "DPS", "student_attack": "DPS", "bloodlust": "DPS",
    "frenzy_clash": "DPS",
    # Special — unique mechanics that don't fit a normal attack/support shape
    "schizo": "Special", "steal_ability": "Special", "gorge": "Special",
    "i_always_get_what_i_want": "Special", "sleepwalker": "Special",
    "weaken": "Special", "claim": "Special", "recognition": "Special",
    "brainwash": "Special", "summon_sister": "Special", "summon_reserve": "Special",
    # Support — buffs, heals, protection for yourself or allies
    "heal": "Support", "abstain": "Support", "purify": "Support", "fast": "Support",
    "moderate": "Support", "gift_power": "Support", "return_ability": "Support",
    "inspire": "Support", "rouse": "Support", "de_escalate": "Support",
    "absorb_strike": "Support", "bless": "Support", "forgive": "Support",
    "endure": "Support", "fortify": "Support", "invoke_faith": "Support",
    "prayer": "Support", "rally": "Support", "beacon": "Support",
    "grant_freedom": "Support", "defend": "Support",
    # Technical — investigative / meta / non-combat utility
    "envy_check": "Technical", "marks": "Technical", "discern": "Technical",
    "wise_counsel": "Technical", "submit": "Technical", "counter_claim": "Technical",
    "scale_of_wrongdoing": "Technical", "redistribution": "Technical",
    "break_chains": "Technical", "inspire_strike": "Technical", "despair_wave": "Technical",
    "llorona_wail": "Technical", "llorona_veil": "Technical", "llorona_lure": "Technical",
}

def get_ability_category(cmd_name: str) -> str:
    return ABILITY_CATEGORIES.get(cmd_name, "Uncategorized")

@bot.command(name="ability_category", aliases=["ability_categories"])
async def ability_category(ctx, category: str = None):
    """List every ability tagged with a category: Attack, AoE/Ultimate, DPS, Special, Support, Technical.
    Run with no argument to see the category names."""
    valid = ["Attack", "AoE/Ultimate", "DPS", "Special", "Support", "Technical"]
    if not category:
        await ctx.send(embed=discord.Embed(
            title="📚 Ability Categories",
            description=(
                "`!ability_category <name>` to list abilities in a category.\n\n"
    ...
    match = None
    for c in valid:
        if c.lower() == category.lower().replace("ultimate", "aoe/ultimate").replace("aoe", "aoe/ultimate"):
            match = c
            break
    if not match:
        await ctx.send(f"❌ Unknown category. Valid: {', '.join(valid)}", delete_after=10)
        return
    cmds = sorted(k for k, v in ABILITY_CATEGORIES.items() if v == match)
    await ctx.send(embed=discord.Embed(
        title=f"📚 {match} Abilities",
        description="\n".join(f"`!{c}`" for c in cmds) or "*(none tagged yet)*",
        color=discord.Color.blurple(),
    ))


# ═══════════════════════════════════════════════════════════════════
# ░░ !abilities — self-service viewer for YOUR current role's powers ░░
# ═══════════════════════════════════════════════════════════════════

_SIN_ABILITY_SUMMARY: dict[str, list[tuple[str, str]]] = {
    "lust":     [("!obsess @user", "Mark a player as your obsession target."),
                 ("!obsession_clash @user", "Clash fueled by obsession.")],
    "gluttony": [("!gorge", "Build clash-coin bonus for 15 min."),
                 ("!feast @user", "Curse target to post food emojis or take a penalty."),
                 ("!devour @user", "Silence a target for 5-10 min based on their power."),
                 ("!purge_bite @user [count]", "Delete a target's recent messages."),
                 ("!expel @user [minutes]", "Temporarily time out and voice-disconnect a target.")],
    "greed":    [("!steal_ability @user", "Clash to steal their primary ability for 5-10 min."),
                 ("!i_always_get_what_i_want @user buff/debuff", "Frenzy-only: force a buff/debuff."),
                 ("!frenzy_clash @user", "Frenzy-only: one buffed clash.")],
    "wrath":    [("!rage_strike @user", "High-damage clash strike."),
                 ("!bloodlust", "+1 coin to all clashes for 1 hour."),
                 ("!summon_meteor @user", "Ultimate — always lands, no clash.")],
    "sloth":    [("!force_lazy @user", "Force abbreviated typing for 30 min."),
                 ("!slowdown @user", "-1 coin on their next clash."),
                 ("!force_sleep @user", "Lock their abilities for 15 min."),
                 ("!deep_sleep @user", "Extended 30-min ability lockout."),
                 ("!sleepwalker", "+1 coin, but you self-slow.")],
    "envy":     [("!jealousy_mark @user", "Ping @everyone with a jealousy mark."),
                 ("!envy_check", "Clear a mark placed on you."),
                 ("!schizo @user", "Flood chat with fake messages. No clash — always lands.")],
    "pride":    [("!weaken <sin>", "Reduce a sin's effective power for 30 min."),
                 ("!claim @user", "Force lower-power sins to bow or gain a mark."),
                 ("!stop_time freeze/@user | passive", "Freeze a target, or auto-evade the next hit.")],
    "gooner":   [("!flash @user", "Stun a target for 30 seconds."),
                 ("!withered_meat @user", "Lock their abilities."),
                 ("!diane_foxington", "Summon Diane Foxington.")],
}

# ═══════════════════════════════════════════════════════════════════
# ░░ ABILITY MENU BUTTONS ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
# Buttons are a MENU only — clicking one never picks a target or fires the
# ability itself. It just tells the clicker (privately, via an ephemeral
# reply) exactly what to type; they still type the @mention themselves.
# Only the person the menu belongs to gets a real answer when they click.
# ═══════════════════════════════════════════════════════════════════

class _AbilityHintButton(discord.ui.Button):
    def __init__(self, label: str, command_hint: str, owner_id: int):
    ...
    async def callback(self, interaction: discord.Interaction):
        if interaction.user.id != self.owner_id:
            await interaction.response.send_message(
                "This menu isn't yours — run the command yourself to get your own.",
                ephemeral=True,
            )
            return
        await interaction.response.send_message(
            f"Type this yourself, filling in `@user` where needed:\n`{self.command_hint}`",
            ephemeral=True,
        )

class AbilityMenuView(discord.ui.View):
    """Up to 25 buttons (5 rows of 5). Only `owner_id` gets a real reply on click."""
    def __init__(self, owner_id: int, entries: list[tuple[str, str]], timeout: float = 180):
        super().__init__(timeout=timeout)
        for command_hint, desc in entries[:25]:
            label = command_hint.split(" ")[0].lstrip("!")
            self.add_item(_AbilityHintButton(label, command_hint, owner_id))

def build_ability_view(owner_id: int, sin: str) -> Optional["AbilityMenuView"]:
    entries = _SIN_ABILITY_SUMMARY.get(sin)
    if not entries:
        return None
    return AbilityMenuView(owner_id, entries)


@bot.command(name="abilities")
async def abilities(ctx):
    """Show YOUR current abilities — whatever sin, virtue, or standalone role you hold.
    Includes clickable buttons; clicking one privately tells you (and only you) the exact
    command to type — it does not target anyone for you."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    save_data(data)

    embed = discord.Embed(title=f"📖 {ctx.author.display_name}'s Abilities", color=discord.Color.blurple())
    found_any = False
    view = None

    sin = user.get("sin_role")
    if sin and sin in _SIN_ABILITY_SUMMARY and not user.get("fallen"):
        lines = "\n".join(f"**{c}** — {d}" for c, d in _SIN_ABILITY_SUMMARY[sin])
        embed.add_field(name=f"{sin.capitalize()} (Sin)", value=lines, inline=False)
        found_any = True
        view = build_ability_view(ctx.author.id, sin)
        for v_sin in user.get("completed_virtues", []):
            if v_sin == sin:
                embed.add_field(
                    name=f"{VIRTUES[v_sin]['virtue']} (Virtue unlocked)",
                    value=f"Run `!{VIRTUES[v_sin].get('cmd_hint', 'commands')}` — see `!commands` for the full Virtue section, or `!ability_category Support` for a shortlist.",
                    inline=False,
                )

    if user.get("path"):
        embed.add_field(
            name=f"Path: {user['path'].capitalize()}",
            value="Use `!path_tutorial` for the full path power list.",
            inline=False,
        )

    if user.get("character"):
        embed.add_field(
            name=f"Character: {user['character'].capitalize()}",
            value="`!mycharacter` for stats, `!talent_ability` / `!talent_kit` to act.",
            inline=False,
        )
        found_any = True
    ...
    if not found_any:
        embed.description = (
            "You don't currently hold a sin, path, or character with special abilities.\n"
            "Run `!sinslist` to see how to obtain one, or `!tutorial` for a full walkthrough."
        )
    else:
        embed.set_footer(text="Universal: !attack, !heal, !bounty, !pact — always available. Buttons below are just a menu — only you get a real answer when you click.")
    if view:
...
async def ability_menu(ctx):
    """Browse ALL ability categories as clickable buttons — same private-menu behavior as !abilities."""
    valid = ["Attack", "AoE/Ultimate", "DPS", "Special", "Support", "Technical"]

    class _CategoryButton(discord.ui.Button):
        def __init__(self, cat_name: str, owner_id: int):
            super().__init__(label=cat_name, style=discord.ButtonStyle.primary)
        ...
        async def callback(self, interaction: discord.Interaction):
            if interaction.user.id != self.owner_id:
                await interaction.response.send_message(
                    "This menu isn't yours — run `!menu` yourself to get your own.",
                    ephemeral=True,
                )
                return
            cmds = sorted(k for k, v in ABILITY_CATEGORIES.items() if v == self.cat_name)
            listing = "\n".join(f"`!{c}`" for c in cmds) or "*(none tagged yet)*"
            await interaction.response.send_message(
                f"**{self.cat_name} abilities:**\n{listing}\n\n"
                "Type the one you want yourself — filling in `@user` where needed.",
                ephemeral=True,
            )

    class _CategoryMenuView(discord.ui.View):
        def __init__(self, owner_id: int):
            super().__init__(timeout=180)
            for c in valid:
                self.add_item(_CategoryButton(c, owner_id))

    await ctx.send(
        embed=discord.Embed(
            title="📚 Ability Menu",
            description="Click a category — the reply is private to you and just shows what to type.",
            color=discord.Color.blurple(),
        ),
        view=_CategoryMenuView(ctx.author.id),
    )


# ═══════════════════════════════════════════════════════════════════
# ░░ SHOP SYSTEM ░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░░
# Currency ("wallet") is earned by completing sin/virtue trials (+75) and
# PVP kills (+25). Everyone starts with 100.
# ═══════════════════════════════════════════════════════════════════

SHOP_ITEMS: dict[str, dict] = {
    "coin_boost":     {"price": 40,  "desc": "+1 clash coin for your next 3 clashes (30 min window).", "needs_target": False},
    "cooldown_wipe":  {"price": 60,  "desc": "Clear ALL of your active ability cooldowns right now.", "needs_target": False},
    "hp_potion":      {"price": 25,  "desc": "Instantly heal 40 HP (stacks with !heal's own cooldown).", "needs_target": False},
    "silence":        {"price": 80,  "desc": "Mute a target for 5 minutes (Discord timeout).", "needs_target": True},
    "delete_message": {"price": 20,  "desc": "Delete a target's most recent message in this channel.", "needs_target": True},
    "gift_role":      {"price": 150, "desc": "Grant a target one of YOUR sin trial roles for 3 minutes (role-play flavor only — no real power transfer).", "needs_target": True},
    "bounty_hit":      {"price": 50,  "desc": "Place a bounty on a target (same effect as free !bounty, just paid).", "needs_target": True},
}

@bot.command(name="wallet", aliases=["balance", "coins"])
async def wallet(ctx):
    """Check your shop currency balance."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    save_data(data)
    await ctx.send(embed=discord.Embed(
        title="👛 Wallet",
        description=f"{ctx.author.mention} has **{user.get('wallet', 0)}** coins.",
        color=discord.Color.gold(),
    ))

@bot.command(name="shop")
async def shop(ctx):
    """View the shop — buy small perks or pay for effects on other players."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    save_data(data)
    embed = discord.Embed(
        title="🛒 The Shop",
        description=f"Your balance: **{user.get('wallet', 0)}** coins. Buy with `!buy <item> [@target]`.",
        color=discord.Color.gold(),
    )
    for key, item in SHOP_ITEMS.items():
        tgt = " *(needs a @target)*" if item["needs_target"] else ""
        embed.add_field(name=f"`{key}` — {item['price']} coins", value=item["desc"] + tgt, inline=False)
    await ctx.send(embed=embed)

@bot.command(name="buy")
async def buy(ctx, item: str, target: discord.Member = None):
    """Purchase a shop item. Usage: !buy <item> [@target]"""
    data = load_data()
    user = get_user(data, ctx.author.id)

    item = item.lower()
    if item not in SHOP_ITEMS:
        await ctx.send(f"❌ Unknown item. Run `!shop` to see options.", delete_after=8)
        save_data(data); return

    info = SHOP_ITEMS[item]
    if info["needs_target"] and target is None:
        await ctx.send(f"❌ `{item}` needs a target: `!buy {item} @user`.", delete_after=8)
        save_data(data); return

    balance = user.get("wallet", 0)
    if balance < info["price"]:
        await ctx.send(
            f"❌ You need **{info['price']}** coins for `{item}` — you have **{balance}**.",
            delete_after=10,
        )
        save_data(data); return

    if target and target.id == ctx.author.id and item in ("silence", "delete_message", "bounty_hit"):
        await ctx.send("❌ You cannot target yourself with that item.", delete_after=8)
        save_data(data); return

    user["wallet"] = balance - info["price"]
    ch = await trial_channel(ctx.guild) or ctx.channel
    result_line = ""

    if item == "coin_boost":
        user["clash_power_bonus"] = (user.get("clash_power_bonus") or 0) + 1
        user["clash_power_bonus_until"] = now_ts() + 30 * 60
        result_line = f"{ctx.author.mention} gained **+1 clash coin** for 30 minutes."

    elif item == "cooldown_wipe":
        for dk in ["gluttony_ability_cds", "greed_ability_cds", "wrath_ability_cds",
                   "sloth_ability_cds", "envy_ability_cds", "gooner_ability_cds",
                   "justice_ability_cds", "virtue_ability_cds", "prudence_ability_cds",
                   "fortitude_ability_cds", "faith_ability_cds", "hope_ability_cds",
                   "liberality_ability_cds", "myth_ability_cds", "path_ability_cds"]:
            user[dk] = {}
        result_line = f"{ctx.author.mention}'s ability cooldowns are **fully cleared**."

    elif item == "hp_potion":
        stats = get_role_stats(ctx.author)
        user["hp"] = min(stats[0], user.get("hp", stats[0]) + 40)
        result_line = f"{ctx.author.mention} healed **40 HP**."
        ...
        async def _apply_silence():
            ...
            try:
                await target.timeout(until, reason=f"!buy silence by {ctx.author}")
            except Exception:
                pass
        asyncio.create_task(_apply_silence())
        result_line = (
            f"{ctx.author.mention} paid to **silence** {target.mention} for 5 minutes "
            f"(after a {grace_secs}s free-message grace period)."
        )

    elif item == "delete_message":
        deleted = False
        try:
            ...
            pass
        if not deleted:
            user["wallet"] = balance  # refund — nothing found to delete
            await ctx.send("❌ No recent message from that target found here. Refunded.", delete_after=10)
            save_data(data); return
        result_line = f"{ctx.author.mention} paid to delete one of {target.mention}'s messages."

    elif item == "gift_role":
        sin = user.get("sin_role")
        if not sin:
            user["wallet"] = balance  # refund — nothing to gift
            await ctx.send("❌ You don't hold a sin role to gift. Refunded.", delete_after=10)
            save_data(data); return
        role_obj = discord.utils.get(ctx.guild.roles, name=SINS[sin]["role"])
        if not role_obj:
            user["wallet"] = balance
            await ctx.send("❌ That trial role doesn't exist yet on this server. Refunded.", delete_after=10)
            save_data(data); return
        try:
            await target.add_roles(role_obj, reason=f"!buy gift_role by {ctx.author}")
        except Exception:
            user["wallet"] = balance
            await ctx.send("❌ Could not assign that role. Refunded.", delete_after=10)
            save_data(data); return
        result_line = f"{ctx.author.mention} gave {target.mention} the **{role_obj.name}** role for 3 minutes."

        async def _revoke():
            await asyncio.sleep(180)
            try:
                await target.remove_roles(role_obj, reason="Gifted trial role expired")
            except Exception:
                pass
        asyncio.create_task(_revoke())

    elif item == "bounty_hit":
        target_id = str(target.id)
        sin_held = None
        for s, holder in data["claimed_sins"].items():
            if holder == target_id:
        ...
        if not sin_held:
            user["wallet"] = balance
            await ctx.send(f"❌ {target.display_name} doesn't hold a sin — bounties only work on sin holders. Refunded.", delete_after=10)
            save_data(data); return
        data.setdefault("bounties", {}).setdefault(target_id, []).append({
            "placer_id": str(ctx.author.id), "expires": now_ts() + 72 * 3600,
        })
        result_line = f"{ctx.author.mention} paid to place a bounty on {target.mention}."

    save_data(data)
    await ch.send(embed=discord.Embed(
        title="🛒 Purchase Complete",
        description=f"{result_line}\n\nRemaining balance: **{user['wallet']}** coins.",
        color=discord.Color.gold(),
    ))


# ═══════════════════════════════════════════════════════════════════
# ░░ PATH SYSTEM — forced 3-minute choice window ░░░░░░░░░░░░░░░░░░
# The 5 existing paths (support/attack/hybrid/tacht/reverence) and all
# their commands are unchanged — this only adds the missing piece: pick
# one within 3 minutes of gaining a role, or DEFAULT_PATH is assigned.
# ═══════════════════════════════════════════════════════════════════

async def start_path_choice_window(member: discord.Member, data: dict):
    """Call whenever someone is granted a sin/virtue/character role for the first time."""
    user = get_user(data, member.id)
    if user.get("path"):
        return  # already has one
    user["path_choice_deadline"] = now_ts() + PATH_CHOICE_WINDOW
    save_data(data)
    try:
        await member.send(
            f"⚔️ You have **3 minutes** to `!choose_path <support|attack|hybrid|tacht|reverence>` "
            f"before **{DEFAULT_PATH}** is assigned to you automatically."
        )
    except Exception:
        pass

@tasks.loop(seconds=30)
async def path_default_check():
    """Assigns DEFAULT_PATH to anyone whose 3-minute choice window has expired."""
    data = load_data()
    changed = False
    for uid, u in data.get("users", {}).items():
        deadline = u.get("path_choice_deadline")
        if not deadline:
            continue
        if u.get("path"):
            u["path_choice_deadline"] = None
            changed = True
            continue
        if now_ts() >= deadline:
            u["path"] = DEFAULT_PATH
            u["path_change_cd"] = now_ts() + PATH_CHANGE_COOLDOWN
            u["path_choice_deadline"] = None
            changed = True
            for guild in bot.guilds:
                member = guild.get_member(int(uid))
                if member:
                    try:
                        await member.send(
                            f"⏱️ Your 3-minute window expired — you've been assigned the "
                            f"**{DEFAULT_PATH.upper()}** path by default. Change it anytime "
                            "with `!choose_path <path>` (7-day lock applies once changed)."
                        )
                    except Exception:
                        pass
                    break
...
# ═══════════════════════════════════════════════════════════════════
# ░░ BANISHMENT — "The Land of Impurity" ░░░░░░░░░░░░░░░░░░░░░░░░░░
# Replaces the old "death = Fallen" flow for PVP/PVE deaths. Roles are kept.
# ═══════════════════════════════════════════════════════════════════

BANISHED_ROLE_NAME     = "Banished"
BANISHED_CHANNEL_NAME  = "banished-place"
BANISHED_VC_NAME       = "Banished Void"

async def send_to_banishment(member: discord.Member, data: dict, reason: str):
    """Apply the Banished role (server-wide mute except in #banished-place), a wallet
    penalty, and a temporary clash debuff. No existing roles are removed."""
    guild = member.guild
    role_obj = discord.utils.get(guild.roles, name=BANISHED_ROLE_NAME)
    if role_obj:
        try:
            await member.add_roles(role_obj, reason=f"Banished: {reason}")
        except Exception:
            pass

    user = get_user(data, member.id)
    lost = min(user.get("wallet", 0), 20)
    user["wallet"] = user.get("wallet", 0) - lost
    user["clash_power_penalty"] = max(user.get("clash_power_penalty", 0), 1)
    user["clash_penalty_until"] = now_ts() + 3600
    user["banished_pry_cd"]     = 0
    save_data(data)

    ch = discord.utils.get(guild.text_channels, name=BANISHED_CHANNEL_NAME)
    if ch:
        await ch.send(embed=discord.Embed(
            title="⛓️ Cast to the Land of Impurity",
            description=(
                f"{member.mention} has been banished ({reason}).\n\n"
                f"You keep every role you hold, but you can only speak in this channel "
                f"(and join its voice room) until you `!pry` your way out.\n"
                f"Lost **{lost}** coins on the way down."
            ),
            color=discord.Color.dark_gray(),
        ))

@bot.command(name="pry")
async def pry(ctx):
    """Attempt to pry your way out of banishment. 2-minute cooldown, ~35% chance."""
    data = load_data()
    user = get_user(data, ctx.author.id)
    role_obj = discord.utils.get(ctx.guild.roles, name=BANISHED_ROLE_NAME)
    if not role_obj or role_obj not in ctx.author.roles:
        await ctx.send("You are not banished.", delete_after=5)
        save_data(data); return

    cd = user.get("banished_pry_cd", 0) or 0
    if now_ts() < cd:
        await ctx.send(f"You can try again in {remaining_fmt(cd)}.", delete_after=8)
        save_data(data); return

    user["banished_pry_cd"] = now_ts() + 2 * 60
    save_data(data)

    if random.random() < 0.35:
        try:
            await ctx.author.remove_roles(role_obj, reason="Pried out of banishment")
        except Exception:
            pass
        await ctx.send(embed=discord.Embed(
            title="⛓️ Freed",
            description=f"{ctx.author.mention} claws their way out of the Land of Impurity.",
            color=discord.Color.green(),
        ))
    else:
        await ctx.send("You claw at the dark, but it holds you fast. Try again in 2 minutes.", delete_after=10)

@bot.command(name="release_banished")
@commands.has_permissions(administrator=True)
async def release_banished(ctx, member: discord.Member):
    """Admin: immediately release a member from banishment."""
    role_obj = discord.utils.get(ctx.guild.roles, name=BANISHED_ROLE_NAME)
    if role_obj and role_obj in member.roles:
        await member.remove_roles(role_obj, reason=f"!release_banished by {ctx.author}")
    await ctx.send(f"✅ {member.mention} has been released from banishment.")

@tasks.loop(minutes=5)
async def banishment_vc_check():
    """Every 5 minutes, banished members in a voice channel have a 10% chance of a 30s VC mute."""
    for guild in bot.guilds:
        role_obj = discord.utils.get(guild.roles, name=BANISHED_ROLE_NAME)
        if not role_obj:
            continue
        for member in role_obj.members:
            if member.voice and member.voice.channel and random.random() < 0.10:
                try:
                    await member.edit(mute=True, reason="Banishment — random VC mute")
                except Exception:
                    continue
                async def _unmute(m=member):
                    await asyncio.sleep(30)
                    try:
                        await m.edit(mute=False, reason="Banishment VC mute expired")
                    except Exception:
                        pass
...
# ═══════════════════════════════════════════════════════════════════
# ░░ JUSTICE TRIAL SYSTEM — "Rebuttal Showdown" ░░░░░░░░░░░░░░░░░░░
# Rule-based (keyword-scoring) judgment engine — there is no real machine
# learning model running here, this file has no ML framework or training
# loop. It plays the role an "AI judge" would via deterministic scoring so
# it behaves consistently and can't be silently reconfigured mid-game.
# One trial can be active per server at a time.
# ═══════════════════════════════════════════════════════════════════

TRIAL_NEGATIVE_KEYWORDS = {
    "lied", "liar", "lying", "cheated", "cheater", "betrayed", "betrayal",
    "stole", "stealing", "thief", "attacked first", "started it", "admit",
    "confessed", "guilty", "sabotaged", "backstabbed", "manipulated",
    "threatened", "harassed", "broke the rules", "cheating", "faked",
    "framed", "hid it", "covered up", "denied it", "ignored", "abused",
}
TRIAL_POSITIVE_KEYWORDS = {
    "helped", "defended", "protected", "innocent", "honest", "truthful",
    "apologized", "warned", "stopped", "de-escalated", "de escalated",
    "cooperated", "admitted fault", "took responsibility", "kind",
    "loyal", "trustworthy", "proof", "witness", "confirmed", "verified",
}

def _evidence_score(quote: str) -> int:
    """Rule-based scoring for a single piece of submitted evidence."""
    lower = quote.lower()
    neg = sum(1 for k in TRIAL_NEGATIVE_KEYWORDS if k in lower)
    pos = sum(1 for k in TRIAL_POSITIVE_KEYWORDS if k in lower)
    score = neg * 12 - pos * 6
    if len(quote) > 60:
        score += 5  # detailed evidence carries a little more weight
    return max(0, score)

def _recompute_blackened(t: dict):
    black = {t["accuser_id"]: 0, t["defendant_id"]: 0}
    for e in t["evidence"]:
        if e.get("disproven"):
            continue
        black[e["against"]] = min(100, black.get(e["against"], 0) + e["score"])
    t["blackened"] = black

def _resolve_trial_scores(t: dict) -> tuple[str, str]:
    """Returns (winner, reason) where winner is 'accuser' or 'defendant'."""
    jurors = t.get("jurors", {})
    acc_votes = sum(1 for v in jurors.values() if v == "accuser")
    def_votes = sum(1 for v in jurors.values() if v == "defendant")
    if abs(acc_votes - def_votes) > 4:
        return ("accuser" if acc_votes > def_votes else "defendant"), "the jurors were overwhelmingly biased"

    black_def = t["blackened"].get(t["defendant_id"], 0)
    black_acc = t["blackened"].get(t["accuser_id"], 0)
    closing = t.get("closing", {})
    close_def = _evidence_score(closing[t["defendant_id"]]) if closing.get(t["defendant_id"]) else 0
    close_acc = _evidence_score(closing[t["accuser_id"]]) if closing.get(t["accuser_id"]) else 0

    case_for_accuser   = black_def + close_acc
    case_for_defendant = black_acc + close_def

    if case_for_accuser == case_for_defendant:
        return random.choice(["accuser", "defendant"]), "the case was a dead heat"
    winner = "accuser" if case_for_accuser > case_for_defendant else "defendant"
    return winner, "the stronger case and closing argument"

async def _conclude_justice_trial(guild: discord.Guild, guild_id: str, outcome: str):
    d = load_data()
    t = d.get("justice_trials", {}).get(guild_id)
    if not t or t.get("status") == "resolved":
        return
    t["status"] = "resolved"
    ch = guild.get_channel(t.get("channel_id")) if t.get("channel_id") else None
    accuser   = guild.get_member(int(t["accuser_id"]))
    defendant = guild.get_member(int(t["defendant_id"]))

    if outcome == "struck_down":
        du = get_user(d, int(t["defendant_id"]))
        du["invulnerable_until"] = now_ts() + 6 * 60
        save_data(d)
        if ch:
            await ch.send(embed=discord.Embed(
                title="⚖️ Trial Struck Down",
                description=(
                    f"A Justice-role holder struck down this trial.\n"
                    f"{defendant.mention if defendant else 'The accused'} is cleared and "
                    "**invulnerable for 6 minutes**."
                ),
                color=discord.Color.blue(),
            ))
        return
    ...
    if outcome == "unfair":
        ...
        if ch:
            await ch.send(embed=discord.Embed(
                title="⚖️ Ruled Unfair",
                description="The judge ruled this trial unfair. It is voided — no consequences for either side.",
                color=discord.Color.blue(),
            ))
        return

    if outcome == "defendant_accepted_banishment":
        save_data(d)
        if defendant:
            await send_to_banishment(defendant, d, "accepted banishment while awaiting a judge")
        if ch:
            await ch.send(embed=discord.Embed(
                title="⚖️ Banishment Accepted",
                description=(
                    f"{defendant.mention if defendant else 'The accused'} chose banishment rather "
                    "than wait any longer for the judge."
                ),
                color=discord.Color.dark_gray(),
            ))
        return

    # "timeout" or a "fair" ruling both fall through to scoring
    winner, why = _resolve_trial_scores(t)
    loser_id = t["defendant_id"] if winner == "accuser" else t["accuser_id"]
    loser = guild.get_member(int(loser_id))
    save_data(d)
    if ch:
        await ch.send(embed=discord.Embed(
            title="⚖️ Verdict Reached",
            description=(
                f"**The {'accuser' if winner == 'accuser' else 'defendant'} wins** — {why}.\n"
                f"{loser.mention if loser else 'The losing side'} is sent to the Land of Impurity."
            ),
            color=discord.Color.dark_red(),
        ))
    if loser:
        await send_to_banishment(loser, d, f"lost a Justice trial ({why})")


@bot.command(name="call_trial")
async def call_trial(ctx, defendant: discord.Member, judge: discord.Member = None, start_minutes: int = 3):
    """Summon someone to a Justice trial. Usage: !call_trial @defendant [@judge] [start_minutes 1-10].
    Preferably name a Justice-role holder as judge — not required, but only a Justice-role
    holder can later !strike_trial it down."""
    data = load_data()
    guild_id = str(ctx.guild.id)
    trials = data.setdefault("justice_trials", {})
    existing = trials.get(guild_id)
    if existing and existing.get("status") != "resolved":
        await ctx.send("⚖️ A trial is already active on this server.", delete_after=8)
        save_data(data); return
    if defendant.id == ctx.author.id:
        await ctx.send("You cannot put yourself on trial as the accuser.", delete_after=6)
        save_data(data); return

    start_minutes = max(1, min(10, start_minutes))
    trial_length  = random.randint(120, 240)  # 2-4 minute showdown

    trial = {
        "status":          "pending",
        "accuser_id":      str(ctx.author.id),
        "defendant_id":    str(defendant.id),
        "judge_id":        str(judge.id) if judge else None,
        "lawyers":         {},
        "called_at":       now_ts(),
        "start_ts":        now_ts() + start_minutes * 60,
        "trial_length":    trial_length,
        "evidence_end_ts": None,
        "evidence":        [],
        "blackened":       {str(ctx.author.id): 0, str(defendant.id): 0},
        "jurors":          {},
        "closing":         {},
        "channel_id":      ctx.channel.id,
    }
    trials[guild_id] = trial
    save_data(data)

    judge_note = f"Judge summoned: {judge.mention}" if judge else "No judge named — any Justice-role holder may still `!strike_trial` it."
    await ctx.send(embed=discord.Embed(
        title="⚖️ A Trial Is Called",
        description=(
            f"{ctx.author.mention} summons {defendant.mention} before the court.\n\n"
            f"{judge_note}\n"
            f"Proceedings begin in **{start_minutes} minute(s)**, then run for "
            f"**{trial_length // 60}m {trial_length % 60}s**.\n\n"
            "`!evidence \"quote\" @user` once it begins • `!disprove <#>` to contest evidence • "
            "`!juror_support @user` if you're watching • `!appoint_lawyer @user` • "
            "`!closing_argument <text>` before time runs out • `!trial_status` any time."
        ),
        color=discord.Color.gold(),
    ))

    async def _start_evidence_phase():
        await asyncio.sleep(start_minutes * 60)
        d = load_data()
        t = d.get("justice_trials", {}).get(guild_id)
        if not t or t["status"] != "pending":
            return
        t["status"] = "evidence"
        t["evidence_end_ts"] = now_ts() + trial_length
        save_data(d)
        ch = ctx.guild.get_channel(t["channel_id"]) or ctx.channel
        await ch.send(embed=discord.Embed(
            title="⚖️ The Trial Begins",
            description=f"Evidence phase is open for **{trial_length // 60}m {trial_length % 60}s**.",
            color=discord.Color.gold(),
        ))
        asyncio.create_task(_auto_resolve())

    async def _auto_resolve():
        await asyncio.sleep(trial_length)
        d = load_data()
        t = d.get("justice_trials", {}).get(guild_id)
        if not t or t["status"] != "evidence":
            return
        await _conclude_justice_trial(ctx.guild, guild_id, "timeout")

    asyncio.create_task(_start_evidence_phase())


@bot.command(name="evidence")
async def evidence_cmd(ctx, *, arg: str):
    """Submit evidence during the evidence phase. Usage: !evidence "quoted text" @user"""
    data = load_data()
    guild_id = str(ctx.guild.id)
    t = data.get("justice_trials", {}).get(guild_id)
    if not t or t["status"] != "evidence":
        await ctx.send("⚖️ No trial is currently accepting evidence.", delete_after=6)
        save_data(data); return

    uid = str(ctx.author.id)
    lawyer_ids = set(t.get("lawyers", {}).values())
    if uid not in (t["accuser_id"], t["defendant_id"]) and uid not in lawyer_ids:
        await ctx.send("⚖️ Only the accuser, defendant, or their lawyers may submit evidence.", delete_after=8)
        save_data(data); return

    m = re.search(r'"([^"]+)"', arg)
    if not m or not ctx.message.mentions:
        await ctx.send('⚖️ Usage: `!evidence "quoted text" @user`', delete_after=8)
        save_data(data); return

    quote  = m.group(1)
    against = ctx.message.mentions[0]
    if str(against.id) not in (t["accuser_id"], t["defendant_id"]):
        await ctx.send("⚖️ Evidence must be filed against the accuser or defendant.", delete_after=8)
        save_data(data); return

    score = _evidence_score(quote)
    eid = len(t["evidence"]) + 1
    t["evidence"].append({
        "id": eid, "by": uid, "against": str(against.id),
        "quote": quote, "score": score, "disproven": False,
    })
    _recompute_blackened(t)
    save_data(data)

    await ctx.send(embed=discord.Embed(
        title=f"📜 Evidence #{eid} Filed",
        description=(
            f"*\"{quote}\"*\n\n"
            f"Filed against {against.mention} — weight **{score}**.\n"
            f"Blackened bar: **{t['blackened'].get(str(against.id), 0)}/100**"
        ),
        color=discord.Color.dark_gold(),
    ))
    if t["blackened"].get(str(against.id), 0) >= 100:
        await ctx.send(
            f"⚫ {against.mention} is **fully blackened** — silenced in this server "
            "except for `!closing_argument`, `!trial_status`, and `!accept_banishment` "
            "until the trial ends."
        )

@bot.command(name="disprove")
async def disprove(ctx, evidence_id: int):
    """Jurors or the opposing side can disprove a piece of evidence, halving its weight."""
    data = load_data()
    guild_id = str(ctx.guild.id)
    t = data.get("justice_trials", {}).get(guild_id)
    if not t or t["status"] != "evidence":
        await ctx.send("⚖️ No trial is currently active.", delete_after=6)
        save_data(data); return
    entry = next((e for e in t["evidence"] if e["id"] == evidence_id), None)
    if not entry:
        await ctx.send("⚖️ No evidence with that number.", delete_after=6)
        save_data(data); return
    if entry["disproven"]:
        await ctx.send("⚖️ That evidence is already fully disproven.", delete_after=6)
        save_data(data); return
    entry["score"] = entry["score"] // 2
    if entry["score"] <= 0:
        entry["disproven"] = True
    _recompute_blackened(t)
    save_data(data)
    await ctx.send(f"⚖️ Evidence #{evidence_id} weight reduced to **{entry['score']}**.")

@bot.command(name="juror_support")
async def juror_support(ctx, side: discord.Member):
    """Onlookers back a side. Usage: !juror_support @accuser_or_defendant"""
    data = load_data()
    guild_id = str(ctx.guild.id)
    t = data.get("justice_trials", {}).get(guild_id)
    if not t or t["status"] != "evidence":
        await ctx.send("⚖️ No trial is currently active.", delete_after=6)
        save_data(data); return
    if str(side.id) not in (t["accuser_id"], t["defendant_id"]):
        await ctx.send("⚖️ You can only support the accuser or the defendant.", delete_after=6)
        save_data(data); return
    if str(ctx.author.id) in (t["accuser_id"], t["defendant_id"]):
        await ctx.send("⚖️ Participants cannot vote as jurors.", delete_after=6)
        save_data(data); return
    which = "accuser" if str(side.id) == t["accuser_id"] else "defendant"
    t["jurors"][str(ctx.author.id)] = which
    save_data(data)
    await ctx.send(f"🧑‍⚖️ {ctx.author.mention} sides with the **{which}**.", delete_after=8)

@bot.command(name="appoint_lawyer")
async def appoint_lawyer(ctx, lawyer: discord.Member):
    """Appoint someone to submit evidence/closing arguments on your behalf."""
    data = load_data()
    guild_id = str(ctx.guild.id)
    t = data.get("justice_trials", {}).get(guild_id)
    if not t or t["status"] == "resolved":
        await ctx.send("⚖️ No trial is currently active.", delete_after=6)
        save_data(data); return
    if str(ctx.author.id) not in (t["accuser_id"], t["defendant_id"]):
        await ctx.send("⚖️ Only the accuser or defendant may appoint a lawyer.", delete_after=6)
        save_data(data); return
    t.setdefault("lawyers", {})[str(ctx.author.id)] = str(lawyer.id)
    save_data(data)
    await ctx.send(f"⚖️ {lawyer.mention} is now representing {ctx.author.mention}.")

@bot.command(name="closing_argument")
async def closing_argument(ctx, *, text: str):
    """Submit your closing argument. Required before the trial can conclude fairly."""
    data = load_data()
    guild_id = str(ctx.guild.id)
    t = data.get("justice_trials", {}).get(guild_id)
    if not t or t["status"] != "evidence":
        await ctx.send("⚖️ No trial is currently in its evidence/closing phase.", delete_after=6)
        save_data(data); return
    uid = str(ctx.author.id)
    side = None
    if uid == t["accuser_id"] or t.get("lawyers", {}).get(t["accuser_id"]) == uid:
        side = t["accuser_id"]
    elif uid == t["defendant_id"] or t.get("lawyers", {}).get(t["defendant_id"]) == uid:
        side = t["defendant_id"]
    if not side:
        await ctx.send("⚖️ Only the accuser, defendant, or their lawyer may give a closing argument.", delete_after=8)
        save_data(data); return
    t.setdefault("closing", {})[side] = text
    save_data(data)
    await ctx.send(
        f"⚖️ Closing argument recorded for the **{'accuser' if side == t['accuser_id'] else 'defendant'}**. "
        "Everyone gets this chance regardless of the blackened bar."
    )

@bot.command(name="judge_rule")
async def judge_rule(ctx, ruling: str, *, reason: str = ""):
    """The named judge only. Usage: !judge_rule fair|unfair [reason]"""
    data = load_data()
    guild_id = str(ctx.guild.id)
    t = data.get("justice_trials", {}).get(guild_id)
    if not t or t["status"] not in ("evidence", "pending"):
        await ctx.send("⚖️ No trial is currently active.", delete_after=6)
        save_data(data); return
    if not t.get("judge_id") or str(ctx.author.id) != t["judge_id"]:
        await ctx.send("⚖️ Only the named judge may issue a ruling.", delete_after=8)
        save_data(data); return
    ruling = ruling.lower()
    if ruling not in ("fair", "unfair"):
        await ctx.send("⚖️ Usage: `!judge_rule fair|unfair [reason]`", delete_after=8)
        save_data(data); return
    save_data(data)
    await _conclude_justice_trial(ctx.guild, guild_id, "unfair" if ruling == "unfair" else "timeout")

@bot.command(name="strike_trial")
async def strike_trial(ctx):
    """Only the Justice-role holder who summoned this trial may strike it down entirely."""
    data = load_data()
    guild_id = str(ctx.guild.id)
    t = data.get("justice_trials", {}).get(guild_id)
    if not t or t["status"] == "resolved":
        ...
        save_data(data); return
    user = get_user(data, ctx.author.id)
    is_summoner  = str(ctx.author.id) == t["accuser_id"]
    has_justice  = _justice_check(user)
    if not (is_summoner and has_justice):
        await ctx.send("⚖️ Only the Justice-role holder who called this trial may strike it down.", delete_after=8)
        save_data(data); return
    save_data(data)
    await _conclude_justice_trial(ctx.guild, guild_id, "struck_down")

@bot.command(name="accept_banishment")
async def accept_banishment_cmd(ctx):
    """If the named judge hasn't shown up, the defendant can accept banishment rather than wait."""
    data = load_data()
    guild_id = str(ctx.guild.id)
    t = data.get("justice_trials", {}).get(guild_id)
    if not t or t["status"] == "resolved":
        await ctx.send("⚖️ No trial is currently active.", delete_after=6)
        save_data(data); return
    if str(ctx.author.id) != t["defendant_id"]:
        await ctx.send("⚖️ Only the defendant may accept banishment.", delete_after=6)
        save_data(data); return
    save_data(data)
    await _conclude_justice_trial(ctx.guild, guild_id, "defendant_accepted_banishment")

@bot.command(name="trial_status")
async def trial_status(ctx):
    """View the current state of the active trial."""
    data = load_data()
    guild_id = str(ctx.guild.id)
    t = data.get("justice_trials", {}).get(guild_id)
    save_data(data)
    if not t or t["status"] == "resolved":
        await ctx.send("⚖️ No active trial.", delete_after=8)
        return
    accuser   = ctx.guild.get_member(int(t["accuser_id"]))
    defendant = ctx.guild.get_member(int(t["defendant_id"]))
    judge     = ctx.guild.get_member(int(t["judge_id"])) if t.get("judge_id") else None
    embed = discord.Embed(title="⚖️ Trial Status", color=discord.Color.gold())
    embed.add_field(name="Accuser", value=accuser.mention if accuser else "?", inline=True)
    embed.add_field(name="Defendant", value=defendant.mention if defendant else "?", inline=True)
    embed.add_field(name="Judge", value=judge.mention if judge else "*(none named)*", inline=True)
    embed.add_field(name="Status", value=t["status"], inline=True)
    embed.add_field(name="Blackened — Accuser", value=f"{t['blackened'].get(t['accuser_id'], 0)}/100", inline=True)
    embed.add_field(name="Blackened — Defendant", value=f"{t['blackened'].get(t['defendant_id'], 0)}/100", inline=True)
    acc_j = sum(1 for v in t.get("jurors", {}).values() if v == "accuser")
    def_j = sum(1 for v in t.get("jurors", {}).values() if v == "defendant")
    embed.add_field(name="Jurors", value=f"Accuser: {acc_j} | Defendant: {def_j}", inline=False)
    if t["status"] == "evidence" and t.get("evidence_end_ts"):
        embed.add_field(name="Time Left", value=remaining_fmt(t["evidence_end_ts"]), inline=False)
    await ctx.send(embed=embed)


@bot.event
async def on_ready():
    # Reconnects can fire on_ready more than once. Starting an already-running
    # loop raises RuntimeError and can leave later maintenance jobs disabled.
    for loop in (
        trial_expiry_check,
        despair_timer_check,
        disaster_resolution_check,
        banishment_vc_check,
        path_default_check,
    ):
    ...
    print(f"\n{'═'*55}")
    print(f"  {BOT_NAME} — Seven Sins Trial System — Online as {bot.user}")
    print(f"{'═'*55}")
    ...
    print(f"  • {HOPE_ROLE}")
    print(f"  • {FEMBOY_ROLE} (optional, opt-in only)")
    print("\nRequired Channels:")
