...
class _AbilityTargetModal(discord.ui.Modal):
    """Collect a target mention or ID for abilities that need one."""

    def __init__(self, panel, ability_key: str):
        display = ABILITY_SPEC[ability_key][1]
        super().__init__(title=f"Target for {display}"[:45], timeout=300)
        self.panel = panel
        self.ability_key = ability_key
        self.target_input = discord.ui.TextInput(
            label="Mention the target or paste their user ID",
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        target_id = int(match.group(1) if match and match.lastindex else match.group(0)) if match else None
        target = guild.get_member(target_id) if guild and target_id else None
        if target is None:
            await interaction.response.send_message(
                "I couldn't find that member in this server. Use a real @mention or member ID.",
                ephemeral=True,
            )
            return
        ...
        for index, (ability_key, display, category, _cd, _effect, _duration, needs_target) in enumerate(
            ...
            async def callback(interaction: discord.Interaction, key=ability_key, needs_t=needs_target):
                if key in TEXT_INPUT_ABILITIES:
                    ...
                    await interaction.response.send_modal(_AbilityTargetModal(self, key))
                else:
